...
const create = async (data) => {
    ...
    return await apiCaller(`/api/products`, "POST", formData, {'Content-Type': 'multipart/form-data'})
}
...
const update = async (id, data) => {
    ...
    if(data.images?.length) {
        data.images.forEach((item) => formData.append('imagers', item))
    }
...
const ProductApi = {
    find, findById, create, update, delete: _delete
}
